...
export class PermissionsGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if (!user || !user.id) {
      throw new UnauthorizedException('User not identified. Please login first.');
    }

    // 3. Gọi RBAC Service để kiểm tra quyền (Core Logic)
    console.log(`🛡️ Checking permission for User [${user.id}]: requires [${requiredPermission.action}] on [${requiredPermission.subject}]`);
